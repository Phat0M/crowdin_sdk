...
- Run the following command to generate Crowdin localization:
  ...
  As a result, the `crowdin_localizations.dart` will be created next to the file generated by Flutter `gen_l10n`: in `output-dir` (or in `arb-dir` if `output-dir` is not specified).
  Make sure this directory is inside `lib/` so you can import it from your code.
...
- `gen_l10n` generates Dart files into `output-dir` (or `arb-dir` if `output-dir` is omitted). Prefer keeping it under `lib/` (for example `output-dir: lib/l10n`) so imports work in a straightforward way.
